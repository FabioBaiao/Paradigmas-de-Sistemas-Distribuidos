...
*/
    ...
    private static class RequestProcessingTask implements Runnable {
        ...
        @Override
        public void run() {
            Order ord = request.getOrd();

            switch (ord.getType()) {
                case BUY:
                    buy(ord.getUser(), ord.getCompany(), ord.getQuantity(), ord.getUnitPrice());
                    break;
                case SELL:
                    sell(ord.getUser(), ord.getCompany(), ord.getQuantity(), ord.getUnitPrice());
                    break;
                default:
